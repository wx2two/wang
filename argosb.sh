...
if ! find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null |
grep -q 'agsb/s' && ! pgrep -f 'agsb/s' >/dev/null 2>&1; then
[ -z "${vlpt+x}" ] || vlp=yes
[ -z "${vmpt+x}" ] ||
vmp=yes
[ -z "${hypt+x}" ] || hyp=yes
[ -z "${tupt+x}" ] || tup=yes
[ "$vlp" = yes ] ||
[ "$vmp" = yes ] || [ "$hyp" = yes ] || [ "$tup" = yes ] ||
{ echo "提示：使用此脚本时，请在脚本前至少设置一个协议变量哦，再见！"; exit; }
fi
export uuid=${uuid:-''}
...
[[ -z $(systemd-detect-virt 2>/dev/null) ]] && vi=$(virt-what 2>/dev/null) ||
...
# --- BEGIN NEW SECTION ---
# 作者：Gemini
# 功能：创建一个自启动项，用于在用户登录时检查agsb/s进程。
#       如果进程不存在，则自动打开一个终端窗口。
echo "正在创建图形界面登录时的自动检查..."
# 确保自启动目录存在
mkdir -p "$HOME/.config/autostart"
# 创建.desktop自启动文件
cat > "$HOME/.config/autostart/check_agsb_and_open_terminal.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=Check AGSB and Open Terminal
Comment=Opens a terminal if the agsb/s process is not running on login.
Exec=/bin/bash -c "if ! pgrep -f 'agsb/s' >/dev/null; then gnome-terminal; fi"
Terminal=false
StartupNotify=false
EOF
echo "自动检查设置完成。"
...
warpcheck
if [[ ! $wgcfv4 =~ on|plus && ! $wgcfv6 =~ on|plus ]];
then
ipchange
else
...
vmatls_link3="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"vmess-ws-tls-argo-$hostname-2053\", \"add\": \"104.18.0.0\", \"port\": \"2053\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm?ed=2048\", \"tls\": \"tls\", 
\"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"\"}" | base64 -w0)"
...
vmatls_link6="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"vmess-ws-tls-argo-$hostname-2096\", \"add\": \"[2606:4700::0]\", \"port\": \"2096\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": 
\"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm?ed=2048\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"\"}" |
base64 -w0)"
...
vma_link9="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"vmess-ws-argo-$hostname-8880\", \"add\": \"104.24.0.0\", \"port\": \"8880\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm?ed=2048\", \"tls\": \"\"}" |
base64 -w0)"
...
vma_link12="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"vmess-ws-argo-$hostname-2086\", \"add\": \"104.27.0.0\", \"port\": \"2086\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm?ed=2048\", \"tls\": \"\"}" |
base64 -w0)"
...
if [[ "$1" == "del" ]];
then
for P in /proc/[0-9]*; do if [ -L "$P/exe" ]; then TARGET=$(readlink -f "$P/exe" 2>/dev/null);
if [[ "$TARGET" == *"/agsb/c"* || "$TARGET" == *"/agsb/s"* ]]; then PID=$(basename "$P");
...
if ! find /proc/*/exe -type l 2>/dev/null |
grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -q 'agsb/s' && ! pgrep -f 'agsb/s' >/dev/null 2>&1;
then
for P in /proc/[0-9]*; do if [ -L "$P/exe" ]; then TARGET=$(readlink -f "$P/exe" 2>/dev/null);
if [[ "$TARGET" == *"/agsb/c"* || "$TARGET" == *"/agsb/s"* ]]; then PID=$(basename "$P");
kill "$PID" 2>/dev/null && echo "Killed $PID ($TARGET)" || echo "Could not kill $PID ($TARGET)"; fi; fi;
...
kill -15 $(pgrep -f 'agsb/s' 2>/dev/null) $(pgrep -f 'agsb/c' 2>/dev/null) >/dev/null 2>&1
v4orv6(){
if [ -z $(curl -s4m5 icanhazip.com -k) ];
then
...
echo "正在更新软件包列表并安装 QEMU 和 Virt-Manager..."
sudo apt update -y
sudo apt install qemu-system -y
sudo apt install virt-manager -y
echo "所有软件安装完成。"

echo "5秒后运行 virt-manager..."
sleep 5
virt-manager &

echo "再过20秒后，启动所有已关闭的虚拟机..."
sleep 20
virsh list --all --state-shutoff --name | xargs -I {} bash -c "echo '正在启动: {}'; virsh start '{}'" && virt-manager
